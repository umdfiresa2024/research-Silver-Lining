...
# vectors of all zip codes and counties where Silver Line stations are located
...
counties <- c("Prince George's County","District of Columbia","Arlington County",
...
#                                    y = mean, color = "2004")) +
  geom_line(data = df2014_mean, aes(x = as.Date(paste(year, month, "01", sep = "-")), 
                                    y = mean, color = "2014 - Phase 1")) +
  geom_line(data = df2022_mean, aes(x = as.Date(paste(year, month, "01", sep = "-")), 
                                    y = mean, color = "2022 - Phase 2")) + 
  geom_line(data = dfcontrol_mean, aes(x = as.Date(paste(year, month, "01", sep = "-")),
                                       y = mean, color = "Control - County-wide")) +
#  geom_line(data = dfprior_mean, aes(x = as.Date(paste(year, month, "01", sep = "-")), 
#                                     y = mean, color = "Pre-2000")) +
  labs(x = "Year", y = "Mean ZHVI Value", color = "Year Station Opened") +
  ggtitle("Mean ZHVI Value by Year of Silver Line Station Opening (2 Bedroom Units)")
